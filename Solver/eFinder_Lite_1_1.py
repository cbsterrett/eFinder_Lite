...
def solveImage():
    global offset_flag, solve, solvedPos, elapsed_time, solved_radec, solved_altaz, firstStar, solution, cam
    print (cam)
    ...
    with Image.open(captureFile) as img:
        ...
        print('number of centroids:',int(centroids.size/2))
        if centroids.size/2 < 10:
            print('Bad image')
            handpad.display("Bad image",str(int(centroids.size/2))+" centroids", "")
            solve = False
            return
        ...
        #pprint (solution)
        elapsed_time = str(time.time() - start_time)[0:4]
...
if param["Camera Type ('QHY' or 'ASI')"]=='ASI':
    ...
    heightCam = 960
    widthCam = 1280
    if param["Lens focal length"] == '50':
        dataBase = 't3_fov5_mag9'
        camCam = (1280,960,15.4,5.5)
    elif param["Lens focal length"] == '25':
...
status = [
    ...
    param["Camera Type ('QHY' or 'ASI')"]+ ' with ' + str(param["Lens focal length"]) + 'mm lens',
    "up_down(-1)",
    "",
    "",
    "left_right(1)",
